...
#include "causalTreeproto.h"
    ...
    static void
honest_coTest0(const int *dimx, int nnode, int nsplit, const int *dimc, 
                            ...
                            const int *xmiss2, int m, double *coEstimate)
{
    int i, j;
    int n;
    int ncat;
...
next:
   ...
   // mapping to themself or parent:
    mapdx = (int *) ALLOC(nnodemax + 1, sizeof(int));
    for (i = 0; i <= nnodemax + 1; i++) {
      mapdx[i]= -1;
    }
    ...
    for (i = 0; i <= nnodemax; i++) {
      ...
      //base case:
      if (trs[origindx] >= m && cons[origindx] >= m) {
        mapdx[i] = i;
      } else {
        mapdx[i] = mapdx[i/2];
      }
    ...
    for (i = 0; i < n; i++) {
      int mywhere = where[i];
      int origindx = invertdx[mywhere];
      if (treatment2[i] == 1) {
        coEstimate[i] = (trsums[origindx] - y2[i]) * 1.0 / (trs[origindx] - wt2[i]) - consums[origindx] * 1.0 / cons[origindx];
      } else {
        coEstimate[i] = trsums[origindx] * 1.0 / trs[origindx] - (consums[origindx] - y2[i]) * 1.0 / (cons[origindx] - wt2[i]);
      }
    }
}
...
SEXP
honest_coTest(SEXP dimx, SEXP nnode, 
                           ...
                           SEXP xmiss2, SEXP m)
{
    ...
    SEXP coEstimate = PROTECT(allocVector(REALSXP, n));
    honest_coTest0(INTEGER(dimx), asInteger(nnode), asInteger(nsplit),
            ...
            INTEGER(xmiss2), asInteger(m), REAL(coEstimate));
    UNPROTECT(1);
    return coEstimate;
}
